...
func HasUnreadNotices(db *sql.DB, userID int64) (bool, error) {
	var count int
	err := db.QueryRow(
		`SELECT COUNT(*) FROM notices n, users u WHERE u.id = ? AND (u.notice_checked_at IS NULL OR n.created_at > u.notice_checked_at)`, userID,
	).Scan(&count)
	return count > 0, err
}
...
func UpdateNotice(db *sql.DB, id int64, title, body string) error {
	_, err := db.Exec(`UPDATE notices SET title=?, body=?, updated_at=? WHERE id=?`, title, body, time.Now(), id)
	return err
}
